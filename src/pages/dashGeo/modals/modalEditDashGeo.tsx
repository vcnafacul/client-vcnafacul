...
function ModalEditDashGeo({
    ...
    UpdateGeolocationStatus({ body, token })
      .then((_) => {
        ...
        if (body.status === 1)
          ...
          toast.success(
            `Cursinho ${geo.name} atualizado com sucesso: Status - Reprovado`,
            { theme: `dark` }
          );
      })
      .catch((error: Error) => {
  ...
  return (
    <>
      <form
        className="bg-white w-full md:w-fit max-h-[90vh] gap-4 p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 overflow-y-auto scrollbar-hide"
