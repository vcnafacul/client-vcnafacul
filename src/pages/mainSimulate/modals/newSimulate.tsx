...
function NewSimulate({ handleClose, title } : NewSimulateProps ) {
    ...
    const getSimulate = () => {
        getSimuladoById(availableSelected.id as string, token)
            ...
            .catch((error: Error) => {
                toast.error(`Erro ao buscar simulado ${availableSelected.id} - Error ${error.message}`)
            })
    }
    ...
    const getAvailables = useCallback(() => {
        getAvailable(title, token)
            .then(res => {
                if(res.length > 0) {
                    const availables = res.map( item => ({ id: item._id, name: item.nome }))
        ...
        <ModalTemplate>
            <div className="bg-white px-20 pt-20 pb-10 rounded-sm ">
                <div className="flex items-center justify-between gap-4">
                    <Text className="text-left" size="secondary">Simulado {title}</Text>
                ...
                <div className="flex justify-end">
                    <div className="flex max-w-[500px] w-full gap-4">
                        ...
                        <Button disabled={availables.length === 0} hover onClick={getSimulate}>Ok, vamos lá!</Button>
                    </div>
                </div>
            </div>
        </ModalTemplate>
