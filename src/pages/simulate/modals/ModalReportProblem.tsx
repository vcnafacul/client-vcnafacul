    ...
    if (message.length > 0) {
      ...
      const body: ReportDTO = {
        ...
        message: `[${
          questionProblem
            ? selectedOption === 1
              ? radios.question.first
              : radios.question.second
            : selectedOption === 1
            ? radios.platform.first
            : radios.platform.second
        }] - ${message}`,
      };
    ...
    return questionProblem ? (
      <div>
        <Text size="tertiary" className="text-start text-grey">
          Foi um problema com a questão,{" "}
          <span
            style={{ fontSize: "16px", color: "#000", fontWeight: "bolder" }}
          >
            [ {numberQuestion} ]
          </span>{" "}
          ?
