...
export function PartnerPrepInscritionStudentManager() {
    ...
    updateIsFreeInfo(studentId, isFree, token)
      .then(() => {
        const newStudent = students.map((stu) => {
          if (stu.id === studentId) {
            return { ...stu, isento: isFree ? "Sim" : "Não" };
          }
          return stu;
        });
  ...
  const handleApplicationStatusInfo = (
    studentId: string,
    status: StatusEnum
  ) => {
    const id = toast.loading("Atualizando informações...");
    updateApplicationStatus(studentId, status, token)
      .then(() => {
        const newStudent = students.map((stu) => {
          if (stu.id === studentId) {
            return {
              ...stu,
              deferido:
                status === StatusEnum.Approved ? "Deferido" : "Indeferido",
            };
          }
          return stu;
        });
  ...
  const columns: GridColDef[] = [
        ...
        <div className="flex justify-center items-center gap-2 h-8 flex-wrap">
          ...
          {params.row.deferido === "Deferido" && params.row.matriculado === "Não" && (
            <ActionButton
              ...
              descriptionAlert={`Confirme a  ${
                params.row.lista_de_espera === "Não" ? "adição" : "remoção"
              } de ${params.row.nome} ${
                params.row.sobrenome
              } da lista de espera`}
              onConfirm={() =>
          ...
          {params.row.deferido !== "Deferido" && params.row.matriculado === "Não" && (
            <ActionButton
              ...
              onConfirm={() =>
                handleApplicationStatusInfo(params.row.id, StatusEnum.Approved)
              }
              tooltipTitle="Deferir"
            >
          ...
          {params.row.deferido !== "Indeferido" && params.row.matriculado === "Não" && (
            <ActionButton
              ...
              onConfirm={() =>
                handleApplicationStatusInfo(params.row.id, StatusEnum.Rejected)
              }
              tooltipTitle="Indeferir"
            >
              <FaXmark className="h-6 w-6 fill-redError opacity-60 hover:opacity-100" />
            </ActionButton>
          )}
        </div>
      ),
    ...
    { field: "deferido", headerName: "Deferimento", width: 150 },
        ...
        <div className="h-full w-full flex pb-2 flex-col sm:flex-row gap-2 sm:gap-0">
          ...
          <div className="items-end flex flex-wrap gap-1 justify-end flex-1">
            ...
            <div className="flex flex-col md:flex-row">
              <AlertDialogUI title={"t"} description={""} onConfirm={() => {}}>
                <AlertDialogTrigger>
                  <Button hover size="small" typeStyle="accepted">
                    <p className="text-sm">Enviar Email Convocação</p>
                  </Button>
                </AlertDialogTrigger>
              </AlertDialogUI>
            </div>
