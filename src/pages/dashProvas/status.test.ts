...
describe("statusDaProva", () => {
  ...
  it("tudo cadastrado e faltando validar é Em validação", () => {
    expect(statusDaProva(contagens(180, 180, 132))).toEqual(STATUS_EM_VALIDACAO);
  ...
  it("contagem dessincronizada (cadastradas acima do total) não vira Em cadastro", () => {
    ...
    expect(statusDaProva(contagens(180, 190, 100))).toEqual(STATUS_EM_VALIDACAO);
  ...
  it("nenhum dos quatro estados desta tela é vermelho", () => {
    ...
    for (const s of todos) {
      ...
      expect(JSON.stringify(dashV2.status[s.tone]), s.label).not.toContain("red");
    }
  });
