import { getProfilePhotos } from "@/services/urls";
import fetchWrapper from "@/utils/fetchWrapper";

export async function getProfilePhoto(key: string, token: string): Promise<Blob> {
  const response = await fetchWrapper(`${getProfilePhotos}/${key}`, {
    method: "GET",
    headers: {
