import { getDocument } from "@/services/urls";
import fetchWrapper from "@/utils/fetchWrapper";

export async function getDocuments(key: string, token: string): Promise<Blob> {
  const response = await fetchWrapper(`${getDocument}/${key}`, {
    method: "GET",
    headers: {
